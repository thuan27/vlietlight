...
import { CreateCusCountryZoneService } from './create-cus-country-zone.service';
...
@Component({
  ...
  selector: 'create-cus-country-zone',
  templateUrl: './create-cus-country-zone.component.html',
  styleUrls: ['./create-cus-country-zone.component.scss'],
  providers: [ValidationService, ToastyService, UserService]
})
// tslint:disable-next-line:component-class-suffix
export class CreateCusCountryZoneComponent implements OnInit {
  ...
  constructor(
    private _createCusCountryZoneService: CreateCusCountryZoneService,
  ...
  onSubmit() {
    if (this.CountryForm.valid) {
      if (this.action === 'create') {
        this._createCusCountryZoneService.createCountryList(this.CountryForm.value).subscribe((data) => {
        ...
        this._createCusCountryZoneService.updateCountry(this.idCountry, this.CountryForm.value).subscribe((data) => {
          this.toastyService.success(data['message']);
          setTimeout(
            () => {
              this.router.navigate(['apps/master-data/countries-zone']);
  ...
  countryList(event) {
    this._createCusCountryZoneService.countryList(event.target.value).subscribe((data) => {
      this.country = data['data'];
    });
  }

  serviceList() {
    this._createCusCountryZoneService.serviceList().subscribe((data) => {
