...
class CreateCustomeromponent implements OnInit {
	items: FormArray;
	...
	titleGroup;
	customerDetail;
	status = [ { value: 10, name: 'Active' }, { value: 9, name: 'Inactive' } ];
	...
	onSubmit() {
		if (this.CustomerForm.valid) {
			if (this.action === 'create') {
				this._createCustomerService.createCustomer(this.CustomerForm.value).subscribe(
					(data) => {
				...
				this._createCustomerService.updateCustomer(this.CustomerForm.value, this.idCus).subscribe(
					(data) => {
