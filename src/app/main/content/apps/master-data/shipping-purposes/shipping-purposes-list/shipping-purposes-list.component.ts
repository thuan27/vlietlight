...
export class ShippingPurposeListComponent implements OnInit {
  ...
  getList(page = 1, ) {
    ...
    this.shippingPurposeList.subscribe((dataList: any[]) => {
      dataList['data'].forEach((data) => {
        ...
        data['country_id'] = `<a href="#/apps/master-data/shipping-purpose/${data['country_id']}">${data['country_code']}</a>`;
      });
  ...
  update() {
    if (this.selected.length < 1) {
      ...
      this.router.navigateByUrl(`apps/master-data/shipping-purpose/${this.selected[0]['country_id_temp']}/update`);
    }
  }
