	...
	getList(page = 1) {
		...
		this.harmonisedCodesList.subscribe((dataList: any[]) => {
			dataList['data'].forEach((data) => {
				...
				data['hs_code'] = `<a href="#/apps/master-data/harmonised-codes/${data['hs_code_temp']}">${data[
					'hs_code_temp'
				]}</a>`;
			});
	...
	delete() {
		if (this.selected.length < 1) {
			...
			this.dialogRef.afterClosed().subscribe((result) => {
				if (result) {
					this.harmonisedCodesListService
						.deleteHarmonisedCode(this.selected[0]['hs_code_temp'])
						.subscribe((data) => {
