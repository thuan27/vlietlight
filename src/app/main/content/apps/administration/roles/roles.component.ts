...
export class RolesComponent implements OnInit {
	...
	private getRoles() {
		this.rolesService.getRoles().subscribe((response) => {
			let checkRoleAdmin = response['data'].find((o) => o.name === 'Admin');
			if (!checkRoleAdmin) {
	...
	private upgradeGroupsPermission() {
		this.allGroupPermission = this.allGroupPermission.map((groupPermission) => {
			...
			for (const permission of groupPermission['permissions']) {
				groupPermission['extended_permissions'].push({
	...
	update() {
		...
		let updatedRole = {
			code: this.activeRole['code'],
			name: this.activeRole['name'],
			update_name: this.activeRole['name'],
