...
export class AWBComponent implements OnInit {
	...
	doCreateWavePick() {
		...
		if (this.selected.length < 1) {
			...
			const result = this.selected.filter((item) => {
				return item.awb_sts === 'New';
			});
	...
	delete() {
		if (this.selected.length < 1) {
			...
			this.dialogRef.componentInstance.confirmMessage =
				'Are you sure you want to delete this AWB. The corresponding Order will be deleted?';
	...
	pending() {
		if (this.selected.length < 1) {
			...
			this.dialogRef = this.dialog.open(FuseConfirmDialogComponent);
			this.dialogRef.componentInstance.confirmMessage =
				'Are you sure you want to pend this AWB. The corresponding Order will be pended too?';
	...
	complete() {
		if (this.selected.length < 1) {
			...
			this.dialogRef = this.dialog.open(FuseConfirmDialogComponent);
			this.dialogRef.componentInstance.confirmMessage =
				'Are you sure you want to complete this AWB. The corresponding Order will be packing?';
	...
	remove() {
		if (this.selected.length < 1) {
			...
			this.dialogRef = this.dialog.open(FuseConfirmDialogComponent);
			this.dialogRef.componentInstance.confirmMessage = `Are you sure you want to remove this AWB out of WavePick ${this
				.selected[0].awb_code}?`;
	...
	filter() {
		this.dialogRefFilter = this.dialog
			.open(FuseFilterAWBComponent, {
				panelClass: 'contact-form-dialog',
				data: {
