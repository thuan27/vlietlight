...
export class AWBDetailV1Component implements OnInit {
	...
	private detailForm(data) {
		...
		this.AWBForm = this.formBuilder.group({
			...
			awb_type: [ { value: data['awb_type'], disabled: true } ],
	...
	buildDetailChildGroup(data = []) {
		...
		for (let i = 0; i < data.length; i++) {
			const detail = this.formBuilder.group({
				item_id: [ { value: data[i]['item_id'], disabled: true } ],
	...
	private buildForm() {
		this.AWBForm = this.formBuilder.group({
			...
			pick_up_date: [ null ],
			ship_date: [ null ],
			awb_type: [ { value: 1, disabled: true } ],
			charge_weight: [ '' ],
			declared_value: [ '' ],
			price: [ null ],
			freight: [ null ],
	...
	buildChildGroup() {
		return this.formBuilder.group({
			item_id: [ { value: null, disabled: true } ],
	...
	CalculateTheVoulume(i) {
		...
		this.AWBForm.controls['awb_details']['controls'][i]['controls']['volume'].setValue(
	...
	validateWeight(control, i) {
		if (this.AWBForm.value['awb_details'][i]['item_id'] == 1) {
			if (Number(control.target.value) <= 2.5 && Number(control.target.value) >= 0) {
				...
				this.AWBForm.controls['awb_details']['controls'][i]['controls']['weight'].setErrors({
