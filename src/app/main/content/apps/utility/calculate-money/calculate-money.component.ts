...
class CalculateMoneyComponent implements OnInit {
	...
	onSubmit(event) {
		...
		if (this.CalculateForm.valid) {
			...
			if (event.submitter.name === 'calculate') {
				params =
					'?service_id=' +
					this.CalculateForm.value['service_id'] +
					'&reweight=' +
				...
				this.calculateMoneyService.calculateMoney(params).subscribe(
					(response) => {
				...
				params =
					...
					'&type=' +
					this.CalculateForm.value['type'];
				this.calculateMoneyService.calculateMoneyAuto(params).subscribe(
					(response) => {
						this.result = response['net_price'];
						this.CalculateForm.reset();
						this.formDirective.resetForm();
	...
	getService(event) {
		...
		this.calculateMoneyService.getService(data).subscribe(
			(data) => {
	...
	getRangePrice(cusServiceID = '') {
		this.loading = true;
		this.calculateMoneyService.getRangePrice(cusServiceID).subscribe(
			(data) => {
	...
	getCountryZone(event) {
		...
		this.loading = true;
		this.calculateMoneyService.getCountryZone(data).subscribe(
			(data) => {
	...
	getServiceWeightRange(serviceID) {
		this.loading = true;
		this.calculateMoneyService.getServiceWeightRange(serviceID).subscribe(
			(data) => {
