...
@Component({
	selector: 'import-range-price',
	templateUrl: './import-range-price.component.html',
	styleUrls: [ './import-range-price.component.scss' ],
	animations: fuseAnimations,
	encapsulation: ViewEncapsulation.None,
	providers: [ UserService, ToastyService, FileManagerService ]
})
export class ImportRangePriceComponent implements OnInit {
	selected: any;
	pathArr: string[];
	hasImportRangePrice;
	itemFile: any;
	...
	private checkPermission() {
		this._user.GetPermissionUser().subscribe(
			(data) => {
				this.hasImportRangePrice = this._user.RequestPermission(data, 'importRangePrice');
				/* Check orther permission if View allow */
				if (!this.hasImportRangePrice) {
	...
	importFile() {
		...
		xhr.open('POST', `${that.apiConfig.IMPORT_RANGE_PRICE}`, true);
		...
		xhr.onreadystatechange = function() {
			...
			if (xhr.readyState == 4) {
				if (xhr.status === 200) {
					...
					FileSaver.saveAs(blob, 'Range_Price_Error.xlsx');
				} else {
					let msg = '';
