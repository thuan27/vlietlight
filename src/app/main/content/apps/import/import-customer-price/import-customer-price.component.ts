...
@Component({
  selector: 'import-customer-price',
  templateUrl: './import-customer-price.component.html',
  styleUrls: ['./import-customer-price.component.scss'],
  animations   : fuseAnimations,
  encapsulation: ViewEncapsulation.None,
  providers: [UserService, ToastyService, FileManagerService],

})
export class ImportCustomerPriceComponent implements OnInit {
  selected: any;
  pathArr: string[];
  hasImportCustomerPrice;
  itemFile:any;
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportCustomerPrice = this._user.RequestPermission(data, 'importCustomerPrice');
        /* Check orther permission if View allow */
        if (!this.hasImportCustomerPrice) {
  ...
  importFile() {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.open("POST", `${that.apiConfig.IMPORT_CUSTOMER_PRICE}`, true);
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'CustomerPrice_Error.xlsx');
				} else {
					let msg = '';
