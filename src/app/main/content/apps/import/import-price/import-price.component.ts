...
@Component({
  selector: 'import-price',
  templateUrl: './import-price.component.html',
  styleUrls: ['./import-price.component.scss'],
...
export class ImportPriceComponent implements OnInit {
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportPrice = this._user.RequestPermission(data, 'importPrice');
        /* Check orther permission if View allow */
        if (!this.hasImportPrice) {
  ...
  importFile() {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.open("POST", `${that.apiConfig.IMPORT_PRICE}`, true);
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'Price_Error.xlsx');
				} else {
					let msg = '';
