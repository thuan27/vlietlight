...
@Component({
  selector: 'import-customer-country-zone',
  templateUrl: './import-customer-country-zone.component.html',
  styleUrls: ['./import-customer-country-zone.component.scss'],
  animations   : fuseAnimations,
  encapsulation: ViewEncapsulation.None,
  providers: [UserService, ToastyService, FileManagerService],

})
export class ImportCustomerCountryZoneComponent implements OnInit {
  selected: any;
  pathArr: string[];
  hasImportCustomerCountryZone;
  itemFile:any;
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportCustomerCountryZone = this._user.RequestPermission(data, 'importCustomerCountryZone');
        /* Check orther permission if View allow */
        if (!this.hasImportCustomerCountryZone) {
  ...
  importFile() {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.open("POST", `${that.apiConfig.IMPORT_CUSTOMER_COUNTRY_ZONE}`, true);
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'CustomerCountryZone_Error.xlsx');
				} else {
					let msg = '';
