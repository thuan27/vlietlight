...
@Component({
  selector: 'import-order-receivable',
  templateUrl: './import-order-receivable.component.html',
  styleUrls: ['./import-order-receivable.component.scss'],
  animations   : fuseAnimations,
  encapsulation: ViewEncapsulation.None,
  providers: [UserService, ToastyService, FileManagerService],

})
export class ImportOrderReceivableComponent implements OnInit {
  selected: any;
  pathArr: string[];
  hasImportOrderReceivable;
  itemFile:any;
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportOrderReceivable = this._user.RequestPermission(data, 'importCountry');////////////
        /* Check orther permission if View allow */
        if (!this.hasImportOrderReceivable) {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'Order_Receivable_Error.xlsx');
				} else {
					let msg = '';
