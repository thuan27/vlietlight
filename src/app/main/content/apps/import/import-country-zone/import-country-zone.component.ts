...
@Component({
  selector: 'import-country-zone',
  templateUrl: './import-country-zone.component.html',
  styleUrls: ['./import-country-zone.component.scss'],
  animations   : fuseAnimations,
  encapsulation: ViewEncapsulation.None,
  providers: [UserService, ToastyService, FileManagerService],

})
export class ImportCountryZoneComponent implements OnInit {
  selected: any;
  pathArr: string[];
  hasImportCountryZone;
  itemFile:any;
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportCountryZone = this._user.RequestPermission(data, 'importCountryZone');
        /* Check orther permission if View allow */
        if (!this.hasImportCountryZone) {
  ...
  importFile() {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.open("POST", `${that.apiConfig.IMPORT_COUNTRY_ZONE}`, true);
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'Country_Zone_Error.xlsx');
				} else {
					let msg = '';
