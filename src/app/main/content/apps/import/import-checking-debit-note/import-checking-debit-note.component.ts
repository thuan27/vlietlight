...
@Component({
  selector: 'import-checking-debit-note',
  templateUrl: './import-checking-debit-note.component.html',
  styleUrls: ['./import-checking-debit-note.component.scss'],
  animations   : fuseAnimations,
  encapsulation: ViewEncapsulation.None,
  providers: [UserService, ToastyService, FileManagerService],

})
export class ImportCheckingDebitNoteComponent implements OnInit {
  selected: any;
  pathArr: string[];
  hasImportCheckingDebitNote;
  itemFile:any;
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportCheckingDebitNote = this._user.RequestPermission(data, 'importCountry');////////////
        /* Check orther permission if View allow */
        if (!this.hasImportCheckingDebitNote) {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'Checking_Debit_Note_Error.xlsx');
				} else {
					let msg = '';
