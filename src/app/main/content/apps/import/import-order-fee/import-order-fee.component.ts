...
@Component({
  selector: 'import-order-fee',
  templateUrl: './import-order-fee.component.html',
  styleUrls: ['./import-order-fee.component.scss'],
  animations   : fuseAnimations,
  encapsulation: ViewEncapsulation.None,
  providers: [UserService, ToastyService, FileManagerService],

})
export class ImportOrderFeeComponent implements OnInit {
  selected: any;
  pathArr: string[];
  hasImportOrderFee;
  itemFile:any;
  ...
  private checkPermission() {
    this._user.GetPermissionUser().subscribe(
      data => {
        this.hasImportOrderFee = this._user.RequestPermission(data, 'importCountry');////////////
        /* Check orther permission if View allow */
        if (!this.hasImportOrderFee) {
      ...
      that.toastyService.error('Only upload file extend .xlsx !!!')
		...
		xhr.onreadystatechange = function() {
          ...
          that.toastyService.error('Import file has some errors.')
					...
					FileSaver.saveAs(blob, 'Order_Fee_Error.xlsx');
				} else {
					let msg = '';
