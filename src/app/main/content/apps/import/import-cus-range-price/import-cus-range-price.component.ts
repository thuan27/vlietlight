...
@Component({
	selector: 'import-cus-range-price',
	templateUrl: './import-cus-range-price.component.html',
	styleUrls: [ './import-cus-range-price.component.scss' ],
	animations: fuseAnimations,
	encapsulation: ViewEncapsulation.None,
	providers: [ UserService, ToastyService, FileManagerService ]
})
export class ImportCusRangePriceComponent implements OnInit {
	selected: any;
	pathArr: string[];
	hasImportCusRangePrice;
	itemFile: any;
	...
	private checkPermission() {
		this._user.GetPermissionUser().subscribe(
			(data) => {
				this.hasImportCusRangePrice = this._user.RequestPermission(data, 'importRangePrice');
				/* Check orther permission if View allow */
				if (!this.hasImportCusRangePrice) {
	...
	importFile() {
		...
		xhr.open('POST', `${that.apiConfig.IMPORT_CUS_RANGE_PRICE}`, true);
		...
		xhr.onreadystatechange = function() {
			...
			if (xhr.readyState == 4) {
				if (xhr.status === 200) {
					...
					FileSaver.saveAs(blob, 'Cus_Range_Price_Error.xlsx');
				} else {
					let msg = '';
